...
visitFunctionParametricAnnotation.test = function(node, path, state) {
  return node.type === Syntax.ParametricTypeAnnotation && path[0] && _isFunctionNode(path[0]) && node === path[0].parametricType;
};
...
visitOptionalFunctionParameterAnnotation.test = function(node, path, state) {
  return node.type === Syntax.OptionalParameter && path[0] && _isFunctionNode(path[0]);
};
...
exports.visitorList = [visitClassProperty, visitFunctionParametricAnnotation, visitFunctionReturnAnnotation, visitOptionalFunctionParameterAnnotation, visitTypeAnnotatedIdentifier];
